...
void push(stack *dst, size_t item);
size_t get_top_of(const stack *src);
void pop(stack *src);
...
#define INITIAL_MEMSIZE 1
  ...
  while ((instruction = fgetc(infile)) != EOF) {
    ...
    case ']':
      if (memory[index] != 0) {
        size_t pos = get_top_of(&loop_stack);
        fsetpos(infile, &pos);
      } else {
        pop(&loop_stack);
      }
...
#define INITIAL_STACK_DEPTH 1
  assert(dst != NULL);
  dst->memory = malloc(sizeof(size_t) * INITIAL_STACK_DEPTH);
...
void push(stack *dst, size_t item) {
  ...
  if (dst->index == dst->size) {
    size_t *new_memory =
        realloc(dst->memory, sizeof(size_t) * (dst->size *= 2));
    if (new_memory == NULL) {
      free(dst->memory);
      failed("failed to allocate memory");
    }
